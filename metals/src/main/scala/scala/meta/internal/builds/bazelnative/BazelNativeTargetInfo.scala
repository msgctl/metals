...
case class BspDependency(
  ...
  def isCompile: Boolean = dependencyType == 0
  def isRuntime: Boolean = dependencyType == 1
}
...
case class BspJvmTargetInfo(
    ...
    transitiveSourceJars: List[BspFileLocation] = Nil,
    transitiveRuntimeJars: List[BspFileLocation] = Nil,
)
...
object BspJvmTargetInfo {
  def fromJson(obj: JsonObject): BspJvmTargetInfo =
    BspJvmTargetInfo(
      ...
      transitiveSourceJars = obj.fileLocList("transitive_source_jars"),
      transitiveRuntimeJars = obj.fileLocList("transitive_runtime_jars"),
    )
}
