...
class BazelNativeBepTranslatorSuite extends FunSuite {
  ...
  private def mkTranslator(
      client: BazelNativeMockClient
  ): BazelNativeBepTranslator = {
  ...
  test("notifyBuildStarted emits TaskStart without targets (fallback)") {
    ...
    val translator = mkTranslator(mockClient)
  ...
  test("onRawStreamEvent returns empty response on invalid data") {
    val mockClient = new BazelNativeMockClient()
    val translator = mkTranslator(mockClient)
